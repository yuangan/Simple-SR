...
class EDVR(nn.Module):
    ...
    def upsampleBlock(self, feat_qk, b, t, h, w):
        ...
        for i in range(self.num_levels-1, -1, -1):
            ...
            if i == 0:
                split_dfq = self.lrelu(self.conv4(dfq_2)).unsqueeze(1)
                dfk = self.lrelu(self.conv4(dfk_2))
                dfq = split_dfq.repeat([1,(t-1),1,1,1])
            else:
                split_dfq = self.lrelu(self.pixel_shuffle(self.upconvs[i-1](dfq_2))).unsqueeze(1)
                dfk = self.lrelu(self.pixel_shuffle(self.upconvs[i-1](dfk_2)))
                dfq = split_dfq.repeat([1,(t-1),1,1,1])
            split_dfq = split_dfq.squeeze(1)
    ...
    def to_image(self, split_dfq, x_center):
        out = self.lrelu(self.pixel_shuffle(self.upconv_final1(split_dfq)))
    ...
    def forward(self, x):
        ...
        split_dfq2, dfk, dfq = self.upsampleBlock(feat_qk1.detach(), b, t, h, w)
        # feat_qk3 = self.pre_recurrent(split_dfq2, dfk, b, t, h, w)
        # split_dfq3, dfk, dfq = self.upsampleBlock(feat_qk3, b, t, h, w)
        out1 = self.to_image(split_dfq1, x_center)
        out2 = self.to_image(split_dfq2, x_center)
        
        out = []
        out.append(out1)
        out.append(out2)
        
        return out2, out, None